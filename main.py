...
def log_stock_quant_metrics(data_dict: dict, params: dict, title: str = "训练集指标快照"):
    """打印每只股票的价格、核心技术指标、量化评分与信号统计（v3.3 - 完整时间序列）"""
    logger.info("\n" + "=" * 80)
    logger.info(f"{title}（完整时间序列）")
    ...
    logger.info("字段: ts_code | trade_date | close | bias20 | rsi14 | macd_hist | score | signal")
    logger.info("=" * 80)
    ...
    for ts_code, df in data_dict.items():
        try:
            ...
            if len(score_series) > 0:
                ...
                score_avg = score_min = score_max = float('nan')
            
            logger.info(f"\n{ts_code} - 总计 {len(sig)} 个交易日 | buy_days={buy_days} | sell_days={sell_days} | score(avg/min/max)={score_avg:.4f}/{score_min:.4f}/{score_max:.4f}")
            
            # 打印每一天的数据（只显示前10天和后10天，中间省略）
            if len(sig) <= 20:
                # 数据少于20天，全部显示
                for idx, row in sig.iterrows():
                    trade_date = str(idx)[:10]
                    close = float(row.get('close', 0) or 0)
                    bias20 = float(row.get('bias20', 0) or 0)
                    ...
                    logger.info(
                        f"  {trade_date} | close={close:.2f} | bias20={bias20:.2f} | "
                        f"rsi14={rsi14:.2f} | macd_hist={macd_hist:.4f} | score={score:.4f} | signal={signal}"
                    )
            else:
                # 数据多于20天，显示前10天和后10天
                ...
                logger.info(f"  ... (省略 {len(sig) - 20} 天) ...")
                
                for idx, row in sig.tail(10).iterrows():
                    trade_date = str(idx)[:10]
                    close = float(row.get('close', 0) or 0)
                    bias20 = float(row.get('bias20', 0) or 0)
                    rsi14 = float(row.get('rsi14', 0) or 0)
            ...
            logger.warning(f"{ts_code} 指标快照失败: {e}", exc_info=True)
