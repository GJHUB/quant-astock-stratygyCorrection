...
def log_stock_quant_metrics(data_dict: dict, params: dict, title: str = "训练集指标快照"):
    ...
    logger.info("字段: ts_code | trade_date | close | bias20 | rsi14 | macd_hist | signal_score(last_valid) | buy_days | sell_days")
    logger.info("=" * 80)
    ...
    for ts_code, df in data_dict.items():
        try:
            ...
            # 解释：score=nan 通常是当日某些技术指标为空导致；这里取最后一个有效分值，避免误读
            score_series = sig['signal_score'].dropna()
            score_last = float(score_series.iloc[-1]) if len(score_series) > 0 else float('nan')
